 ...
 * 
 * Script này hoàn toàn độc lập — kết nối trực tiếp đến SQLite file,
 * KHÔNG import server.js hay bất cứ module nào gọi app.listen().
 * 
 * Mỗi lần chạy sẽ xóa seed data cũ (dựa trên user_id prefix "seed_user_")
 * rồi insert lại từ đầu, đảm bảo không có rác data.
 */

const path = require('path');
const Database = require('better-sqlite3');

// scripts/ → lên 1 cấp = backend/ → mangas.db
const dbPath = path.resolve(__dirname, '../mangas.db');
console.log('[SeedPosts] DB path:', dbPath);
...
try {
    ...
    // ═══════════════════════════════
    // Chạy trong transaction — dọn cũ rồi seed mới
    // ═══════════════════════════════
    const seedAll = db.transaction(() => {
        // 1. Dọn dẹp: xóa posts & likes cũ của seed users
        const seedUserIds = seedUsers.map(u => u.userId);
        const placeholders = seedUserIds.map(() => '?').join(', ');

        // Xóa likes liên quan đến posts của seed users
        const deleteLikes = db.prepare(`
            DELETE FROM post_likes WHERE post_id IN (
                SELECT id FROM posts WHERE user_id IN (${placeholders})
            )
        `);
        const likesDeleted = deleteLikes.run(...seedUserIds);
        console.log(`🧹 Đã xóa ${likesDeleted.changes} seed post likes cũ`);

        // Xóa posts cũ của seed users
        const deletePosts = db.prepare(
            `DELETE FROM posts WHERE user_id IN (${placeholders})`
        );
        const postsDeleted = deletePosts.run(...seedUserIds);
        console.log(`🧹 Đã xóa ${postsDeleted.changes} seed posts cũ`);
    ...
    if (db) {
        db.close();
        console.log('[SeedPosts] Database connection closed.');
    }
}
